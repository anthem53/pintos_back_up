donate_priority_chain(struct lock * lock);

/* Debugging function */
void print_ready_list(struct thread *t ,void * aux);
...
void thread_sleep(int wakeup_time)
{
   enum intr_level old_level;
   struct thread * cur = thread_current();
   old_level = intr_disable();
...
void thread_wake_up (int64_t wake_ticks){
...
void donate_priority_chain(struct lock * lock)
{
  ...
  /*Move holder to front ready and yield. So holder can start immediately */
